 ...
 *
 * ★ PYG-437 — token "ไม่คงที่" อีกต่อไป (ต่างจากที่ PYG-434 ออกแบบไว้ตอนแรก):
 *     · เช็คอินสำเร็จ → token เปลี่ยนเป็นของ CHECK_OUT ทันที (คนละค่ากับตอนเริ่มงาน)
 *     · ผู้รับบริการกด rotateJobQr → token เปลี่ยนทันที ใบเก่าตาย
 *   → FE ต้อง poll/refetch เสมอ ห้ามแคช token ไว้นาน ๆ และห้ามชวนผู้ใช้ปริ้นท์เก็บไว้
 *
...
export class JobQr {
  ...
  @Field({
    description:
      'PYG-437 — QR ชุดที่เห็นอยู่นี้ถูกออกเมื่อไหร่. เปลี่ยนค่าเมื่อ: ผู้รับบริการกดออก QR ใหม่ · เช็คอินสำเร็จ (token ของ "จบงาน" เป็นคนละใบกับ "เริ่มงาน"). FE เอาไปโชว์ว่า "ออก QR ใหม่เมื่อ HH:MM" เพราะตัว QR เปลี่ยนลายนิดเดียว มองด้วยตาแทบไม่ออก',
  })
  tokenIssuedAt: Date;
