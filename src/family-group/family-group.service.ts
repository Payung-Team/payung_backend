...
export class FamilyGroupService {
  ...
  async joinGroupByLink(
    ...
    await this.prisma.$transaction(async (tx) => {
      const updatedLinks = await tx.$queryRaw<{ id: string; group_id: string }[]>`
      ...
      if (updatedLinks.length === 0) {
        ...
        // freshLink มี type ไม่ตรงเป๊ะ แต่ใช้แค่ฟิลด์ที่ต้องการ
        this.assertJoinLinkUsable(freshLink, memberCount);
        throw new JoinLinkInvalidError();
      }

      await tx.familyGroupMember.upsert({
        where: {
          groupId_userId: { groupId: link.groupId, userId },
        },
        create: {
