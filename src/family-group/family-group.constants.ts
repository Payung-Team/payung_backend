 ...
 *
 * 20 มาจากหน้าจอจริง: การ์ดกิจกรรมสูงราว 64px บนจอ 1440 ที่ทีมใช้เป็น canvas
 * → เห็นพร้อมกันได้ราว 10–12 ใบ ขอมา 20 = เลื่อนได้เกือบสองจอก่อนต้องยิงคำขอถัดไป
 * เยอะกว่านี้คือจ่ายค่าถ่ายข้อมูลของแถวที่ผู้ใช้ส่วนใหญ่ไม่เคยเลื่อนไปถึง
 */
export const ACTIVITY_PAGE_SIZE_DEFAULT = 20;

/**
 * เพดานของ first — ขอเกินนี้จะถูก "หั่นลงเงียบ ๆ" ไม่ใช่โยน error
 *
 * ★ ตั้งใจหั่นแทนที่จะปฏิเสธ เพราะเพดานนี้เป็นเรื่องของเซิร์ฟเวอร์ ไม่ใช่ความผิดของ client
 *   หน้าฟีดที่โหลดของมาน้อยกว่าที่ขอยังใช้งานได้ปกติ (มี hasNextPage บอกว่ายังมีต่อ)
 *   แต่หน้าที่พังทั้งหน้าเพราะขอ 500 แถวคือประสบการณ์ที่แย่กว่าโดยไม่ได้อะไรกลับมา
 */
export const ACTIVITY_PAGE_SIZE_MAX = 50;

/**
 * ตัวคั่นภายใน cursor ระหว่าง createdAt กับ id
 *
 * ปลอดภัยเพราะทั้งสองฝั่งเป็นค่าที่มีรูปแบบตายตัว (ISO-8601 กับ UUID)
 * ซึ่งไม่มีทางมีอักขระตัวนี้อยู่ข้างใน → split แล้วไม่มีทางได้ชิ้นส่วนเกิน
 */
export const ACTIVITY_CURSOR_SEPARATOR = '|';
